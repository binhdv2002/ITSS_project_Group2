...
export default function Invoices() {
	...
	useEffect(() => {
		if (!stationId) return;
		axios.get('http://localhost:8080/api/bike-station/detail/' + stationId)
	...
	const handleBackToHome = () => {
		...
		localStorage.setItem('rental-time', JSON.stringify({ hours: 0, minutes:0, seconds:0}))
	}
			...
			<div className='grid grid-cols-2 mt-10'>
				<div>
					...
					<div className='grid grid-cols-2 gap-4'>
						...
						<h2 className='font-medium'>Group 8</h2>
						...
						<h2 className='font-medium'>{currencyFormat(bike?.bikeType?.depositPrice)}</h2>
						<h2 className='font-medium'>Fee</h2>
						<h2 className='font-medium'>{currencyFormat(0)}</h2>
						<h2 className='font-medium'>Total</h2>
						<h2 className='font-medium'>{currencyFormat(newRent?.totalCost)}</h2>
					</div>
				...
				<div className='px-[100px]'>
					...
					>
						<div className='flex items-center justify-center py-2'>
							{loading ? <Spin /> : <span className='flex items-center gap-2'>{returnMessage} <CheckCircleOutlined className='text-green-500 text-[20px]' /></span>}
						</div>
					</Modal>
				</div>
			</div>
